...
class BiasRegLayer(Layer):
    """Like normal layer but with bias regularization"""
    def __init__(self, 
        scope=None, 
        input_dims=None, 
        output_dims=None, 
        ...
        biases_initializer='zeros', 
        reg_initializer=None, 
        bias_reg_initializer=None, 
        activation_scaling=1.75,
        normalize_output=None,
        num_inh=0, 
        pos_constraint=None, 
        log_activations=False):
        ...
        # Add bias regularization
        self.bias_reg = Regularization(
            input_dims=self.filter_dims,
            num_outputs=self.num_filters,
            vals=reg_initializer)

        self.ei_mask = np.array(self.ei_mask)*activation_scaling
        self.normalize_output = normalize_output

    def copy_layer_params(self, origin_layer):
        super().copy_layer_params(origin_layer)
        self.bias_reg = origin_layer.bias_reg.reg_copy()

    def define_regularization_loss(self):
        w_reg = super().define_regularization_loss()
        b_reg = self.bias_reg.define_reg_loss(self.biases_var)
        return tf.add(w_reg,b_reg)

    def assign_reg_vals(self, sess):
        super().assign_reg_vals(sess)
        self.bias_reg.assign_reg_vals(sess)

    def convert_to_unit_regularization( self ):
        super().convert_to_unit_regularization()
        self.bias_reg = UnitRegularization(
            input_dims=self.input_dims,
            num_outputs=self.bias_reg.num_outputs, 
            vals=self.bias_reg.vals)

    def build_graph(self, inputs, params_dict=None, batch_size=None, use_dropout=False):
        super().build_graph(inputs, params_dict, batch_size, use_dropout)
        if self.normalize_output:
            self.outputs = np.sqrt(self.normalize_output)*tf.nn.l2_normalize(self.outputs,axis=1)

class DeconvLayer(Layer):
    """Implementation of deconvolutional layer
    ...
    def __init__(
        ...
        # Changes in properties from Layer - note this is implicitly multi-dimensional
        if output_shape:
            self.output_dims = [num_filters] + output_shape[:]
        else:
            self.output_dims = [num_filters] + num_shifts[:]
    ...
    def build_graph(self, inputs, params_dict=None, batch_size=None, use_dropout=False):
        ...
        with tf.name_scope(self.scope):
            ...
            # Reshape weights (4:D: [filter_height, filter_width, in_channels, out_channels])
            conv_filter_dims = [self.filter_dims[2], self.filter_dims[1], self.num_filters,self.filter_dims[0]
                                ]
            ...
            self.outputs = tf.reshape(
                post_drpd, [-1, np.prod(self.output_dims)])
...
class MaskLayer(Layer):
    ...
    def build_graph(self, inputs, params_dict=None, batch_size=None, use_dropout=False):
        with tf.name_scope(self.scope):
            ...
            if self.normalize_output:
                normalized = (normalized-tf.reduce_mean(normalized,axis=0)) / tf.math.reduce_std(normalized,axis=0) 
                #np.sqrt(self.normalize_output)*tf.nn.l2_normalize(normalized,axis=1)
